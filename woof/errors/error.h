...
#include "woof/io/stream/buffer.h"
...
/**
 * @brief Base class for all exceptions thrown within Woof.
 *
...
template <
  typename ErrorType,
  typename T,
  std::enable_if_t<std::is_base_of_v<Error, ErrorType>>* = nullptr
>
...
ErrorType wrap(
  ...
  static_assert(
    ...
    "Can only wrap with another Woof error class."
  );
  ErrorType err{loc};
