...
constexpr bool can_convert_duration(
  ...
  using destination_units = DestinationUnits;
  using source_units = std::chrono::duration<Rep, Period>;
  if constexpr (std::is_same_v<destination_units, source_units>) return true;
